...
// Demo 1: Shape showcase
void demo1_shapes(void) {
    printf("Demo 1: Shapes\n");

    ra8876_fill_screen(RA8876_BLACK);
    ra8876_print(400, 10, RA8876_WHITE, "Shape Demo");

    // Grid
    ra8876_print(10, 50, RA8876_GRAY, "Grid:");
    for (int x = 100; x < 400; x += 50) {
        ra8876_draw_line(x, 80, x, 280, RA8876_DARKGRAY);
    }
    for (int y = 80; y < 300; y += 50) {
        ra8876_draw_line(100, y, 400, y, RA8876_DARKGRAY);
    }

    // Rectangles
    ra8876_print(450, 50, RA8876_GRAY, "Rectangles:");
    ra8876_draw_rect(450, 80, 101, 71, RA8876_RED);
    ra8876_fill_rect(560, 80, 101, 71, RA8876_GREEN);
    ra8876_draw_rect(670, 80, 101, 71, RA8876_BLUE);
    ra8876_fill_rect(680, 90, 81, 51, RA8876_CYAN);

    // Rounded Rectangles
    ra8876_print(800, 50, RA8876_GRAY, "Rounded:");
    ra8876_draw_rounded_rect(800, 80, 121, 71, 15, RA8876_YELLOW);
    ra8876_fill_rounded_rect(810, 90, 101, 51, 10, RA8876_ORANGE);

    // Circles
    ra8876_print(450, 170, RA8876_GRAY, "Circles:");
    ra8876_draw_circle(500, 240, 40, RA8876_YELLOW);
    ra8876_fill_circle(600, 240, 40, RA8876_MAGENTA);
    ra8876_draw_circle(700, 240, 50, RA8876_WHITE);
    ra8876_fill_circle(700, 240, 30, RA8876_ORANGE);

    // Ellipses
    ra8876_print(10, 320, RA8876_GRAY, "Ellipses:");
    ra8876_draw_ellipse(150, 400, 80, 40, RA8876_CYAN);
    ra8876_fill_ellipse(300, 400, 40, 60, RA8876_RED);
    ra8876_draw_ellipse(450, 400, 60, 30, RA8876_GREEN);
    ra8876_fill_ellipse(450, 400, 40, 20, RA8876_YELLOW);

    // Triangles
    ra8876_print(550, 320, RA8876_GRAY, "Triangles:");
    ra8876_draw_triangle(600, 350, 550, 450, 650, 450, RA8876_WHITE);
    ra8876_fill_triangle(750, 350, 700, 450, 800, 450, RA8876_BLUE);
    ra8876_fill_triangle(900, 350, 850, 450, 950, 450, RA8876_RED);
    ra8876_draw_triangle(900, 350, 850, 450, 950, 450, RA8876_WHITE);

    // Lines
    ra8876_print(10, 480, RA8876_GRAY, "Lines:");
    for (int i = 0; i < 10; i++) {
        uint16_t color = ra8876_rgb565(255 - i*25, i*25, 128);
        ra8876_draw_line(100 + i*30, 510, 100 + i*30 + 100, 580, color);
    }

    // BTE Color Expand (1-bit to 16-bit)
    ra8876_print(500, 480, RA8876_GRAY, "BTE Expand (1bpp->16bpp):");
    ...
    ra8876_bte_expand(0, 500, 510, 32, 32, icon_32x32, RA8876_CYAN, RA8876_BLACK);
    ra8876_bte_expand(0, 540, 510, 32, 32, icon_32x32, RA8876_RED, RA8876_BLACK);
    ra8876_bte_expand(0, 580, 510, 32, 32, icon_32x32, RA8876_GREEN, RA8876_DARKGRAY);
    ra8876_bte_expand(0, 620, 510, 32, 32, icon_32x32, RA8876_YELLOW, RA8876_BLUE);

    ra8876_printf(700, 550, RA8876_WHITE, "%dx%d @ %dbpp",
        RA8876_WIDTH, RA8876_HEIGHT, RA8876_BPP);

    sleep_ms(5000);
}

// Demo 2: Bouncing rectangle screensaver with FPS (double buffered)
void demo2_bounce(void) {
    printf("Demo 2: Bouncing Rectangle (Double Buffered)\n");

    // Initialize double buffering for tear-free animation
    ...
    for (int i = 0; i < 500; i++) {  // Run for 500 frames
        uint32_t frame_start = time_us_32();

        // Clear the back buffer (what we're drawing to)
        ...
        if (y <= 0 || y + h >= RA8876_HEIGHT - 30) {  // Leave room for FPS
            dy = -dy;
            y += dy;
        }

        // Cycle colors (RGB fade)
        switch (color_state) {
        ...
        // Draw rectangle on back buffer
        ra8876_fill_rect(x, y, w, h, color);

        // Draw FPS on back buffer
        ra8876_printf(10, RA8876_HEIGHT - 20, RA8876_WHITE, "FPS: %lu", fps);

        // Swap buffers - instantly shows the completed frame
        ra8876_swap_buffers();
        ...
        // Frame timing (target ~60 FPS = 16.6ms per frame)
    ...
    // Disable double buffering for next demo
    ra8876_buffer_disable();

    printf("Bounce demo complete\n");
}

// Demo 3: Text editor simulation with cursor
void demo3_text(void) {
    ...
    // Title bar
    ra8876_fill_rect(0, 0, RA8876_WIDTH, 31, RA8876_DARKGRAY);
    ra8876_print(10, 8, RA8876_WHITE, "Untitled.txt - Text Editor");

    // Text area background
    ra8876_fill_rect(0, 31, RA8876_WIDTH, RA8876_HEIGHT - 61, RA8876_BLACK);

    // Status bar
    ra8876_fill_rect(0, RA8876_HEIGHT - 30, RA8876_WIDTH, 30, RA8876_DARKGRAY);
    ...
    int cursor_line = 9;  // Line with "    " (empty line in code)
    int cursor_col = 4;
    ...
    for (int i = 0; i < 300; i++) {  // Run for 300 frames
        uint32_t now = time_us_32();
        ...
        if (now - last_blink >= 500000) {
            ...
            if (cursor_visible) {
                ra8876_fill_rect(cursor_x, cursor_y, 3, 17, RA8876_WHITE);
            } else {
                ra8876_fill_rect(cursor_x, cursor_y, 3, 17, RA8876_BLACK);
            }
        }
        ...
        if (now - last_fps_time >= 1000000) {
            ...
            // Update status bar
            ra8876_fill_rect(0, RA8876_HEIGHT - 30, RA8876_WIDTH, 30, RA8876_DARKGRAY);
            ra8876_printf(10, RA8876_HEIGHT - 22, RA8876_WHITE,
...
void demo4_bte(void) {
    ...
    ra8876_fill_rect(10, 10, 81, 81, RA8876_RED);
    ra8876_fill_rect(30, 30, 41, 41, RA8876_YELLOW);
    ra8876_fill_circle(50, 50, 20, RA8876_BLUE);

    ra8876_set_canvas_page(3);
    ra8876_fill_screen(RA8876_BLACK);
    for (int i = 0; i < 10; i++) {
        ra8876_fill_rect(i * 100, 0, 51, 600, ra8876_rgb565(i * 25, 50, 100));
    }
    ...
    for (int i = 0; i < 500; i++) {
        ...
        ra8876_bte_copy_chroma(2, 0, 0, draw_page, sprite_x + 150, sprite_y, 100, 100, RA8876_MAGENTA);
        ra8876_bte_copy_chroma(2, 0, 0, draw_page, sprite_x + 300, sprite_y, 100, 100, RA8876_MAGENTA);

        ra8876_printf(10, 10, RA8876_WHITE, "BTE Demo - FPS: %lu", fps);
        ra8876_printf(10, 30, RA8876_CYAN, "Sprites with chroma key transparency");
...
void demo5_cursor(void) {
    ...
    ra8876_fill_screen(RA8876_BLACK);
    ra8876_fill_rect(0, 0, RA8876_WIDTH, 31, RA8876_DARKGRAY);
    ra8876_print(10, 8, RA8876_WHITE, "Hardware Cursor Demo - Zero CPU Blink");

    ra8876_fill_rect(20, 50, RA8876_WIDTH - 40, RA8876_HEIGHT - 100, ra8876_rgb565(0, 0, 40));
    ...
    ra8876_set_text_cursor(30,60);// + 4 * 8, 60 + 9 * 20);
    ra8876_cursor_size(8, 16);
    ra8876_cursor_blink_rate(30);
    ra8876_cursor_show(true);

    ra8876_print(30, 400, RA8876_YELLOW, "Cursor blinks with no CPU involvement!");
    ra8876_print(30, 430, RA8876_CYAN, "Try ra8876_invert_area() for text selection:");
    ra8876_set_text_cursor(30,60);// + 4 * 8, 60 + 9 * 20);
    ...
    sleep_ms(1000);
    ra8876_invert_area(30, 80, 8 * 45, 20);
...
void demo6_pip(void) {
    ...
    for (int i = 0; i < 20; i++) {
        ra8876_draw_line(0, i * 30, RA8876_WIDTH, i * 30, RA8876_DARKGRAY);
        ra8876_draw_line(i * 60, 0, i * 60, RA8876_HEIGHT, RA8876_DARKGRAY);
    }
    ra8876_print(300, 280, RA8876_WHITE, "Main Display - Background Grid");
    ra8876_print(250, 320, RA8876_GRAY, "PIP windows float above without redrawing this");

    ra8876_set_canvas_page(2);
    ra8876_fill_rect(0, 0, 200, 100, ra8876_rgb565(40, 0, 0));
    ra8876_draw_rect(0, 0, 200, 100, RA8876_RED);
    ra8876_print(10, 10, RA8876_WHITE, "HUD Window 1");
    ra8876_print(10, 35, RA8876_YELLOW, "HP: 100/100");
    ra8876_print(10, 55, RA8876_CYAN, "MP: 50/50");
    ra8876_print(10, 75, RA8876_GREEN, "Gold: 1234");

    ra8876_set_canvas_page(3);
    ra8876_fill_rect(0, 0, 152, 62, ra8876_rgb565(0, 40, 0));
    ra8876_draw_rect(0, 0, 152, 62, RA8876_GREEN);
    ra8876_print(10, 10, RA8876_WHITE, "Minimap");
    ra8876_fill_circle(75, 35, 5, RA8876_RED);
    ra8876_fill_rect(50, 25, 11, 11, RA8876_BLUE);
    ra8876_fill_rect(100, 40, 21, 11, RA8876_YELLOW);

    ra8876_set_canvas_page(0);

    ra8876_pip1_enable(2, 20, 20, 200, 100);
    ra8876_pip2_enable(3, RA8876_WIDTH - 170, 20, 152, 62);

    ra8876_print(300, 550, RA8876_WHITE, "PIP windows move with ZERO background redraw!");
...
void demo7_cgram(void) {
    ...
    ra8876_cgram_init();
    ra8876_cgram_upload_font(oldschool_font_8x16, ' ', 95, 16);
    ...
    ra8876_set_text_cursor(0, 0);
    ra8876_put_cgram_string("OLDSCHOOL FONT DEMO - ASCII 32-126");

    ra8876_set_text_cursor(0, 20);
    ra8876_put_cgram_string("THE QUICK BROWN FOX JUMPS OVER THE LAZY DOG");

    ra8876_set_text_cursor(0, 40);
    ra8876_put_cgram_string("the quick brown fox jumps over the lazy dog");

    ra8876_set_text_cursor(0, 60);
    ra8876_put_cgram_string("0123456789 !@#$%^&*() []{}|;':\",./<>?");

    ra8876_set_text_colors(RA8876_CYAN, ra8876_rgb565(0, 0, 32));
    ra8876_set_text_cursor(0, 100);
    ra8876_put_cgram_string("C:\\>DIR /W");
    ra8876_set_text_cursor(0, 120);
    ra8876_put_cgram_string(" Volume in drive C is RETRO_DOS");
    ra8876_set_text_cursor(0, 140);
    ra8876_put_cgram_string(" Directory of C:\\GAMES");
    ra8876_set_text_cursor(0, 160);
    ra8876_put_cgram_string("DOOM     QUAKE    DUKE3D   HEXEN    HERETIC");

    sleep_ms(3000);

    ra8876_buffer_init(4);
    ...
    for (int f = 0; f < 300; f++) {
        ...
        for (int row = 0; row < rows; row++) {
            ...
            ra8876_set_text_cursor(0, row * 16);
            ra8876_put_cgram_string(line);
        }
    ...
    ra8876_select_internal_font(RA8876_FONT_16, RA8876_ENC_8859_1);
    ra8876_fill_screen(RA8876_BLACK);
    ra8876_printf(10, 10, RA8876_WHITE, "CGRAM Benchmark Complete");
    ra8876_printf(10, 40, RA8876_GREEN, "300 frames: %lu us", elapsed);
    ra8876_printf(10, 70, RA8876_CYAN, "Average FPS: %.2f", avg_fps);
    ra8876_printf(10, 100, RA8876_YELLOW, "Chars/sec: %.0f", chars_per_sec);
    ra8876_printf(10, 130, RA8876_MAGENTA, "Screen: %dx%d = %d chars", cols, rows, total_chars);
...
void demo8_internal_font_bench(void) {
    ...
    for (int f = 0; f < 300; f++) {
        uint8_t draw_page = ra8876_get_draw_page();
//        ra8876_bte_solid_fill(draw_page, 0, 0, RA8876_WIDTH, RA8876_HEIGHT, RA8876_BLACK);
        ra8876_set_text_colors(RA8876_GREEN, RA8876_BLACK);
    ...
    ra8876_fill_screen(RA8876_BLACK);
    ra8876_printf(10, 10, RA8876_WHITE, "Internal Font Benchmark Complete");
    ra8876_printf(10, 40, RA8876_GREEN, "300 frames: %lu us", elapsed);
    ra8876_printf(10, 70, RA8876_CYAN, "Average FPS: %.2f", avg_fps);
    ra8876_printf(10, 100, RA8876_YELLOW, "Chars/sec: %.0f", chars_per_sec);
    ra8876_printf(10, 130, RA8876_MAGENTA, "Screen: %dx%d = %d chars", cols, rows, total_chars);
...
void demo9_vsync_test(void) {
    ...
    ra8876_fill_screen(RA8876_BLACK);
    ra8876_print(10, 10, RA8876_WHITE, "VSYNC Counter Test");
    ra8876_print(10, 40, RA8876_GRAY, "Counting vsync events for 10 seconds...");
    ...
    while (time_us_32() - start_time < 10000000) {
        ...
        if (now - last_update >= 1000000) {
            ...
            ra8876_fill_rect(10, 80, 391, 121, RA8876_BLACK);
            ra8876_printf(10, 80, RA8876_GREEN, "VSYNC count: %lu", vsync_count);
            ra8876_printf(10, 110, RA8876_CYAN, "Elapsed: %lu sec", elapsed_sec);
            ra8876_printf(10, 140, RA8876_YELLOW, "Refresh rate: %.1f Hz", hz);
    ...
    ra8876_printf(10, 200, RA8876_WHITE, "Final: %lu vsyncs = %.2f Hz", vsync_count, final_hz);
...
void demo10_cgram_cursor(void) {
    ...
    ra8876_fill_screen(ra8876_rgb565(0, 0, 40));

    ra8876_select_cgram_font(RA8876_FONT_16);
    ra8876_set_text_colors(RA8876_GREEN, ra8876_rgb565(0, 0, 40));

    ra8876_set_text_cursor(0, 0);
    ra8876_put_cgram_string("RETRO TERMINAL v1.0");

    ra8876_set_text_cursor(0, 20);
    ra8876_put_cgram_string("=====================================");
    ...
    for (int i = 0; i < 14; i++) {
        ra8876_set_text_cursor(0, 50 + i * 18);
        ra8876_put_cgram_string(lines[i]);
    }

    ra8876_set_text_cursor(32, 50 + 13 * 18);
    ra8876_cursor_size(8, 16);
  // ra8876_cursor_blink_rate(30);
   ra8876_cursor_show(true);

    ra8876_set_text_colors(RA8876_YELLOW, ra8876_rgb565(0, 0, 40));
    ra8876_set_text_cursor(0, 350);
    ra8876_put_cgram_string("Hardware cursor blinking - zero CPU!");

    sleep_ms(3000);

    ra8876_set_text_colors(RA8876_CYAN, ra8876_rgb565(0, 0, 40));
    ra8876_set_text_cursor(0, 380);
    ra8876_put_cgram_string("Selecting text with invert_area()...");

    sleep_ms(1000);
    ra8876_invert_area(0, 50 + 4 * 18, 8 * 45, 18);
    sleep_ms(500);
    ra8876_invert_area(0, 50 + 5 * 18, 8 * 45, 18);
    sleep_ms(500);
    ra8876_invert_area(0, 50 + 6 * 18, 8 * 45, 18);

    ra8876_set_text_colors(RA8876_WHITE, ra8876_rgb565(0, 0, 40));
    ra8876_set_text_cursor(0, 420);
    ra8876_put_cgram_string("3 files selected!");
    ...
    ra8876_set_text_cursor(0, 460);
    ra8876_put_cgram_string("Moving cursor across the screen:");

    ra8876_set_text_cursor(0, 500);
    ra8876_put_cgram_string("..................................................");

    for (int i = 0; i < 50; i++) {
        ra8876_set_text_cursor(i * 8, 500);
        ra8876_cursor_show(true);
        sleep_ms(50);
    }
    ...
    ra8876_set_text_cursor(0, 540);
    ra8876_put_cgram_string("Cursor on text: HELLO_WORLD");

    ra8876_set_text_cursor(8 * 21, 540);
    ra8876_cursor_show(true);
...
static void life_draw(uint8_t page) {
    ra8876_bte_solid_fill(page, 0, 40, RA8876_WIDTH, RA8876_HEIGHT - 40, RA8876_BLACK);

    ra8876_bte_batch_start(page, LIFE_CELL_SIZE - 1, LIFE_CELL_SIZE - 1);
    for (int y = 0; y < LIFE_ROWS; y++) {
        for (int x = 0; x < LIFE_COLS; x++) {
            if (life_grid[y][x]) {
                uint16_t px = x * LIFE_CELL_SIZE;
                uint16_t py = 40 + y * LIFE_CELL_SIZE;
                ra8876_bte_batch_fill(px, py, RA8876_GREEN);
            }
        }
    }
...
void demo11_game_of_life(void) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        ra8876_bte_solid_fill(draw_page, 0, 0, RA8876_WIDTH, 40, ra8876_rgb565(0, 0, 60));

        life_draw(draw_page);

        ra8876_set_canvas_page(draw_page);
        ra8876_select_internal_font(RA8876_FONT_16, RA8876_ENC_8859_1);
        ra8876_printf(10, 10, RA8876_WHITE, "Game of Life  Gen: %lu  FPS: %lu  Grid: %dx%d", gen, fps, LIFE_COLS, LIFE_ROWS);
...
static void plat_draw(uint8_t page) {
    ra8876_bte_solid_fill(page, 0, 0, RA8876_WIDTH, RA8876_HEIGHT, ra8876_rgb565(40, 44, 52));

    for (int i = 0; i < 6; i++) {
        int16_t mx = ((i * 200) - (bg_scroll / 3) % 200 + 1200) % 1200 - 100;
        if (mx >= 0 && mx + 60 < RA8876_WIDTH)
            ra8876_bte_solid_fill(page, mx, 80 + i * 15, 60, 40, ra8876_rgb565(60, 64, 72));
    }

    ra8876_bte_solid_fill(page, 0, PLAT_GROUND_Y, RA8876_WIDTH, RA8876_HEIGHT - PLAT_GROUND_Y, ra8876_rgb565(60, 40, 30));
    ra8876_bte_solid_fill(page, 0, PLAT_GROUND_Y, RA8876_WIDTH, 5, ra8876_rgb565(80, 60, 40));
    ...
    for (int i = 0; i < PLAT_MAX_PLATFORMS; i++) {
        ...
        if (px > -platforms[i].w && px < RA8876_WIDTH) {
            ...
            if (draw_w > 0) {
                ra8876_bte_solid_fill(page, draw_x, platforms[i].y, draw_w, platforms[i].h, platforms[i].color);
                ra8876_bte_solid_fill(page, draw_x, platforms[i].y, draw_w, 4, ra8876_rgb565(100, 200, 100));
            }
        }
    }
    ...
    ra8876_bte_solid_fill(page, player.x, player.y + 10, PLAT_PLAYER_W, PLAT_PLAYER_H - 10, body_color);
    ra8876_bte_solid_fill(page, player.x + 4, player.y, 16, 14, head_color);

    int leg_offset = (scroll_x / 4) % 8;
    if (!player.on_ground) leg_offset = 4;
    ra8876_bte_solid_fill(page, player.x + 4, player.y + PLAT_PLAYER_H, 4, leg_offset, body_color);
    ra8876_bte_solid_fill(page, player.x + PLAT_PLAYER_W - 8, player.y + PLAT_PLAYER_H, 4, 8 - leg_offset, body_color);
}
...
void demo12_platformer(void) {
    ...
    for (int i = 0; i < 3000; i++) {
        ...
        ra8876_set_canvas_page(draw_page);
        ra8876_select_internal_font(RA8876_FONT_16, RA8876_ENC_8859_1);
        ra8876_printf(10, 10, RA8876_WHITE, "SCORE: %lu", player.score);
        ra8876_printf(900, 10, RA8876_WHITE, "FPS: %lu", fps);
...
void demo13_blend_write_pip(void) {
    ...
    ra8876_set_canvas_page(2);
    ra8876_fill_screen(ra8876_rgb565(20, 20, 60));
    for (int i = 0; i < 12; i++) {
        uint16_t color = ra8876_rgb565(100 + i * 10, 50, 150 - i * 10);
        ra8876_fill_circle(100 + i * 80, 300, 60 - i * 3, color);
    }
    ra8876_print(350, 50, RA8876_WHITE, "Layer A: Circles");

    ra8876_set_canvas_page(3);
    ra8876_fill_screen(ra8876_rgb565(60, 20, 20));
    for (int i = 0; i < 8; i++) {
        uint16_t color = ra8876_rgb565(200, 100 + i * 15, 50);
        ra8876_fill_rect(50 + i * 120, 100, 51, 401, color);
    }
    ra8876_print(350, 50, RA8876_WHITE, "Layer B: Bars");

    ra8876_set_canvas_page(4);
    ra8876_fill_screen(RA8876_BLACK);
    ra8876_bte_write(4, 0, 0, 128, 128, pixels);
    ra8876_draw_rect(0, 0, 128, 128, RA8876_WHITE);

    ra8876_set_canvas_page(5);
    ra8876_fill_rect(0, 0, 140, 60, ra8876_rgb565(0, 60, 0));
    ra8876_draw_rect(0, 0, 140, 60, RA8876_GREEN);
    ra8876_print(10, 10, RA8876_WHITE, "PIP2 Overlay");
    ra8876_print(10, 35, RA8876_CYAN, "Alpha Blend");
    ...
    ra8876_pip1_enable(4, 100, 200, 128, 128);
    ra8876_pip2_enable(5, 800, 50, 140, 60);
    ...
    for (int i = 0; i < 600; i++) {
        ra8876_bte_blend(2, 0, 0, 3, 0, 0, 0, 0, 0, RA8876_WIDTH, RA8876_HEIGHT, alpha);

        ra8876_set_canvas_page(0);
        ra8876_printf(10, 10, RA8876_WHITE, "Alpha: %3d/255  FPS: %lu", alpha, fps);
        ra8876_print(10, 570, RA8876_GRAY, "bte_write: texture in PIP1 | bte_blend: layer mixing | pip1+pip2: dual floating windows");
...
void demo14_text_transparency(void) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        ra8876_fill_rect(i * 52, 0, 52, RA8876_HEIGHT, color);
    }

    for (int y = 0; y < RA8876_HEIGHT; y += 40) {
        ra8876_draw_line(0, y, RA8876_WIDTH, y, RA8876_DARKGRAY);
    }

    ra8876_print(300, 30, RA8876_WHITE, "Text Transparency Demo");

    ra8876_set_text_transparent(false);
    ra8876_set_text_colors(RA8876_WHITE, RA8876_RED);
    ra8876_set_text_cursor(50, 100);
    ra8876_put_string("OPAQUE TEXT - Background color fills character cells");

    ra8876_set_text_cursor(50, 140);
    ra8876_set_text_colors(RA8876_BLACK, RA8876_YELLOW);
    ra8876_put_string("This text has a solid yellow background");

    ra8876_set_text_transparent(true);
    ra8876_set_text_colors(RA8876_WHITE, RA8876_BLACK);
    ra8876_set_text_cursor(50, 220);
    ra8876_put_string("TRANSPARENT TEXT - Pattern shows through!");

    ra8876_set_text_cursor(50, 260);
    ra8876_set_text_colors(RA8876_YELLOW, RA8876_BLACK);
    ra8876_put_string("Great for HUDs, overlays, and floating labels");

    ra8876_set_text_cursor(50, 300);
    ra8876_set_text_colors(RA8876_CYAN, RA8876_BLACK);
    ra8876_put_string("The background color is ignored when transparent");

    for (int y = 380; y < 500; y += 30) {
        for (int x = 50; x < 900; x += 150) {
            uint16_t color = ra8876_rgb565(rand() % 256, rand() % 256, rand() % 256);
            ra8876_fill_circle(x + rand() % 50, y + rand() % 20, 10 + rand() % 20, color);
        }
    }

    ra8876_set_text_cursor(50, 400);
    ra8876_set_text_colors(RA8876_WHITE, RA8876_BLACK);
    ra8876_put_string("Transparent text over random circles");

    ra8876_set_text_cursor(50, 440);
    ra8876_set_text_colors(RA8876_GREEN, RA8876_BLACK);
    ra8876_put_string("Perfect for game UIs and status displays");

    ra8876_set_text_transparent(false);
    ra8876_set_text_colors(RA8876_WHITE, RA8876_DARKGRAY);
    ra8876_set_text_cursor(50, 520);
    ra8876_put_string("Back to opaque mode - use ra8876_set_text_transparent(bool)");
